...
bootstrap-ir-invariants-only:
	...
	if timeout 15 ./self-host/ir_driver --combined tests/cases/test_imported_struct_fields.fg | awk 'BEGIN { id=0; name=0; bad=0 } /^field / && NF==4 { bad=1 } /^field / && $$5=="int" && $$6=="id" { id=1 } /^field / && $$5=="string" && $$6=="name" { name=1 } END { if (id && name && !bad) exit 0; exit 1 }'; then \
